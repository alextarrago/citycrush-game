...
export let scoreText, levelText, timerText;
...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // const buttonReset = this.add.image(190, INITIAL_BOARD_SCREEN.HEIGHT + 140, 'ButtonReset').setDepth(1).setOrigin(0, 0);
        // buttonReset.setInteractive({ useHandCursor: true });
        // buttonReset.on('pointerup', () => gameManager.reset());

        const buttonMenu = this.add.image(190, INITIAL_BOARD_SCREEN.HEIGHT + 240, 'ButtonMenu').setDepth(1).setOrigin(0, 0);
        buttonMenu.setInteractive({ useHandCursor: true });
        buttonMenu.on('pointerup', () => this.scene.start('MenuScene'));

        timerText = this.add.text(190, INITIAL_BOARD_SCREEN.HEIGHT - 60, 'Temps: --', { font: 'bold 40px Geneva' }).setDepth(1);
        scoreText = this.add.text(190, INITIAL_BOARD_SCREEN.HEIGHT, 'Punts: 0', { font: 'bold 40px Geneva' }).setDepth(1);
        levelText = this.add.text(190, INITIAL_BOARD_SCREEN.HEIGHT + 60, 'Nivell: 1', { font: 'bold 40px Geneva' }).setDepth(1);
